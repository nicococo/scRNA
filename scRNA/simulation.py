...
def split_source_target(toy_data, true_toy_labels,
    ...
    if mode == 1:
        ...
        _, toy_data_source, _, true_toy_labels_source = train_test_split(np.transpose(toy_data_source_exclusive),
                                                                         true_toy_labels_source_exclusive,
                                                                         test_size=source_ncells,
                                                                         stratify=true_toy_labels_source_exclusive)

        _, toy_data_target, _, true_toy_labels_target = train_test_split(np.transpose(toy_data_target_exclusive),
        ...
        if cluster_spec == None:
            if min(true_toy_labels) == 1:
                true_toy_labels = true_toy_labels - 1
                go_back_flag = True
            nclusters, counts = np.unique(true_toy_labels, return_counts=True)
            ntrg = np.int(np.floor((len(nclusters) + common) / 2.))
            nsrc = len(nclusters) - ntrg + common

            assert (nsrc + ntrg - common <= len(nclusters))
            Cidx = np.random.choice(nclusters, common, False)
            if not nsrc - common == 0:
                Sidx = np.random.choice(np.setdiff1d(nclusters, Cidx), nsrc - common,
                                        False)  # Indices of exclusive source clusters
                if not ntrg - common == 0:
                    Tidx = np.random.choice(np.setdiff1d(nclusters, np.union1d(Sidx, Cidx)), ntrg - common,
                                            False)  # Indices of exclusive target clusters
                else:
                    Tidx = []
            else:
                Sidx = []
            ...
            while not (sum(counts[Sidx.astype(int)]) >= source_ncells):
                Sidx = np.append(Sidx, Tidx[0])
                Tidx = np.delete(Tidx, 0)

            if (sum(counts[Tidx.astype(int)]) <= target_ncells):
                target_ncells = sum(counts[Tidx.astype(int)])
        ...
        if len(Cidx) > 0:
            ...
            data_shared_target, data_shared_source, labels_shared_target, labels_shared_source = train_test_split(
                toy_data[:, shared_idx].transpose(), np.array(true_toy_labels)[shared_idx], train_size=shared_trg_size,
                test_size=shared_src_size)
        else:
        ...
        if ntrg > common:
            ...
            if add_trg_size == sum(trg_idx):
                toy_data_target = toy_data[:, trg_idx].transpose()
                true_toy_labels_target = np.array(true_toy_labels)[trg_idx]
            else:
                if add_trg_size > sum(trg_idx):
                    pdb.set_trace()
                toy_data_target, _, true_toy_labels_target, _ = train_test_split(toy_data[:, trg_idx].transpose(),
                                                                                 np.array(true_toy_labels)[trg_idx],
        ...
        if nsrc > common:
            ...
            if add_src_size > sum(src_idx):
                pdb.set_trace()
            elif add_src_size == sum(src_idx):
                toy_data_source = toy_data[:, src_idx].transpose()
                true_toy_labels_source = np.array(true_toy_labels)[src_idx]
            else:
                toy_data_source, _, true_toy_labels_source, _ = train_test_split(toy_data[:, src_idx].transpose(),
                                                                                 np.array(true_toy_labels)[src_idx],
    ...
    if go_back_flag:
        true_toy_labels_source = true_toy_labels_source + 1
        true_toy_labels_target = true_toy_labels_target + 1
