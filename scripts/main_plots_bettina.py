...
def plot_main(fig_num, res, accs_desc, m_desc, percs, genes, n_src, n_trg, common):
    ...
    plt.title('ARI for 1000 src datapts, 500 genes, {0} overlapping clusters, optimal mixture parameter'.format(common[ind_common]), fontsize=16)
...
def plot_unsupervised_measures(fig_num, res, accs_desc, m_desc, percs, genes, n_src, n_trg, common):
    ...
    for c in range(len(accs_desc)-1):
        ...
        plt.xticks(np.arange(aris.shape[1]) + 0.5, ['SC3', 'SC3 Comb', 'SC3 Mix max', 'SC3 Mix min'], rotation=60)
