...
def acc_silhouette(trg_nmf, X_trg, trg_labels, lbls_pred, metric='euclidean'):
    ...
    return metrics.silhouette_score(dists, lbls_pred, metric='precomputed'), \
           'Silhouette ({0})'.format(metric)


def acc_kta(target_nmf, X_trg, trg_labels, trg_lbls_pred, kernel='linear', param=1.0, center=True, normalize=True, mode=0):
    ...
    while r < reps:
        ...
        for i in range(len(percs)):
            ...
            for m in range(len(methods)):
                desc, target_nmf, trg_lbls_pred = methods[m](source_nmf, p_trg.copy(), p_trg_labels.copy(), n_trg_cluster=n_trg_cluster)
                res_desc.append(desc)
                accs_desc = list()
                for f in range(len(acc_funcs)):
                    accs[f, r, i, m], accs_descr = acc_funcs[f](target_nmf, p_trg.copy(), p_trg_labels.copy(), trg_lbls_pred.copy())
